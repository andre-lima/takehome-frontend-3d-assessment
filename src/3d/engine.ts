...
export default class ThreeEngineController {
  ...
  isSceneInitialized: boolean = false;
  ...
  clientToNdc({ clientX, clientY }: { clientX: number; clientY: number }): [number, number] {
    if (!this.renderer) {
  ...
  render() {
    ...
    if (!this.scene || !this.camera || !this.renderer) {
      throw new Error('Scene, camera, renderer, and controls must be defined before rendering');
    }
  ...
  updateSize(canvasElement: HTMLCanvasElement) {
    ...
    adjustObjectsForCanvas({ renderer: this.renderer, camera: this.camera }, canvasElement);
  }
...
function buildCamera(canvasElement: HTMLCanvasElement) {
  ...
  const camera = new PerspectiveCamera(75, isNaN(aspectRatio) ? 1 : aspectRatio, 0.1, 1000);
...
function buildControls(camera: PerspectiveCamera, canvasElement: HTMLCanvasElement) {
